...
# Taken from http://www.roguebasin.com/index.php/A_Simple_Dungeon_Generator_for_Python_2_or_3

# Changes that I made

# A few minior tweaks/updates to the syntax to run in Python 3.6

# Changed the functions to return dungeon in a nested list format
# to suit my needs. 


from __future__ import print_function
from code import util, map, terminal
...
def build_dungeon(p, add_rare=None):
    ...
    gen = Generator(
        ...
        min_room_xy=5,
        max_room_xy=15,
        rooms_overlap=random.choice([False, True]),
    ...
    for i in range(random.randint(4, 15)):
        map.place_rand_object(d, 't')


    util.log("Add Exit")
    map.place_rand_object(d, '@')


    util.log("Adding Enemies")
    for i in range(random.randint(3, 8)):
        enemy = monsters.gen_random()
        map.place_rand_object(d, enemy)
    ...
    if add_rare:
        util.log("Adding Rare")
        map.place_rand_object(d, add_rare)
